...
class NET(torch.nn.Module):
    def __init__(self,
        ...
        # setup network
        self.net = model
        self.sampler = samplers[args.ssm_args['sampler']](args)
    ...
    def observe_class_IL_batch(self, args, g, dataloader, features, labels, t, train_ids, ids_per_cls, dataset):
        ...
        if t!=self.current_task:
            ...
            c_nodes_sampled, nbs_sampled= self.sampler(g, args.ssm_args['c_node_budget'], args.ssm_args['nei_budget'], self.net, ids_per_cls_train)
    ...
    def observe_task_IL_batch(self, args, g, dataloader, features, labels, t, train_ids, ids_per_cls, dataset):
        ...
        for input_nodes, output_nodes, blocks in dataloader:
            ...
            if t != self.current_task:
                self.current_task = t
                c_nodes_sampled, nbs_sampled = self.sampler(g, args.ssm_args['c_node_budget'],
                                                            args.ssm_args['nei_budget'], self.net,
                                                            ids_per_cls_train)
                old_ids = g.ndata['_ID'].cpu()
